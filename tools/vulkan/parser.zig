...
fn parseBitmaskType(ty: *xml.Element) !registry.Declaration {
    if (ty.getAttribute("name")) |name| {
        ...
        const bitwidth: u8 = if (std.mem.eql(u8, flags_type, "VkFlags"))
            32
        else if (std.mem.eql(u8, flags_type, "VkFlags64"))
            64
        else
            return error.InvalidRegistry;
        return registry.Declaration{
...
fn parseHandleType(ty: *xml.Element) !registry.Declaration {
    ...
    if (ty.getAttribute("name")) |name| {
        ...
        const dispatchable = std.mem.eql(u8, handle_type, "VK_DEFINE_HANDLE");
        if (!dispatchable and !std.mem.eql(u8, handle_type, "VK_DEFINE_NON_DISPATCHABLE_HANDLE")) {
            return error.InvalidRegistry;
        }
        return registry.Declaration{
            .name = name,
...
fn parseBaseType(allocator: Allocator, ty: *xml.Element) !registry.Declaration {
    ...
    if (ty.getCharData("type")) |_| {
        var tok = tokenizer.XmlCTokenizer.init(ty);
...
fn parseContainer(allocator: Allocator, ty: *xml.Element, is_union: bool, api: registry.Api) !registry.Declaration {
    ...
    while (it.next()) |member| {
        ...
        var xctok = tokenizer.XmlCTokenizer.init(member);
        members[i] = try tokenizer.parseMember(allocator, &xctok, false);
        if (std.mem.eql(u8, members[i].name, "sType")) {
...
fn parseFuncPointer(allocator: Allocator, ty: *xml.Element) !registry.Declaration {
    var xctok = tokenizer.XmlCTokenizer.init(ty);
    return try tokenizer.parseTypedef(allocator, &xctok, true);
}
...
fn parseEnums(
    ...
    while (it.next()) |enums| {
        ...
        if (std.mem.eql(u8, name, api_constants_name) or !requiredByApi(enums, api)) {
            continue;
        }
...
fn parseEnumFields(allocator: Allocator, elem: *xml.Element, api: registry.Api) !registry.Enum {
    ...
    const bitwidth = if (elem.getAttribute("bitwidth")) |bitwidth| try std.fmt.parseInt(u8, bitwidth, 10) else 32;
...
fn parseCommand(allocator: Allocator, elem: *xml.Element, api: registry.Api) !registry.Declaration {
    ...
    var proto_xctok = tokenizer.XmlCTokenizer.init(proto);
    const command_decl = try tokenizer.parseParamOrProto(allocator, &proto_xctok, false);
    ...
    while (it.next()) |param| {
        ...
        var xctok = tokenizer.XmlCTokenizer.init(param);
        const decl = try tokenizer.parseParamOrProto(allocator, &xctok, false);
        params[i] = .{
            .name = decl.name,
        ...
        if (param.getAttribute("optional")) |optionals| {
            var optional_it = std.mem.splitScalar(u8, optionals, ',');
            if (optional_it.next()) |first_optional| {
                params[i].is_optional = std.mem.eql(u8, first_optional, "true");
            } else {
                // Optional is empty, probably incorrect.
                return error.InvalidRegistry;
            }
...
fn parseDefines(
    ...
    while (it.next()) |ty| {
        ...
        if (std.mem.eql(u8, name, "VK_HEADER_VERSION") or std.mem.eql(u8, name, "VKSC_API_VARIANT")) {
            try api_constants.append(allocator, .{
                .name = name,
                .value = .{ .expr = std.mem.trim(u8, ty.children[2].char_data, " ") },
            });
        } else {
            var xctok = tokenizer.XmlCTokenizer.init(ty);
            try api_constants.append(allocator, .{
                .name = name,
                .value = tokenizer.parseVersion(&xctok) catch continue,
            });
        }
    }
}
...
fn parseRequire(
    ...
    const required_feature_level = blk: {
        ...
        if (!std.mem.startsWith(u8, feature_level, "VK_VERSION_")) {
            return error.InvalidRegistry;
        }
...
fn parseExtension(allocator: Allocator, extension: *xml.Element, api: registry.Api) !registry.Extension {
    ...
    const promoted_to: registry.Extension.Promotion = blk: {
        ...
        if (std.mem.startsWith(u8, promotedto, "VK_VERSION_")) {
...
fn requiredByApi(elem: *xml.Element, api: registry.Api) bool {
    ...
    var it = std.mem.splitScalar(u8, apis, ',');
